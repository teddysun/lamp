...
1、（可选安装）执行脚本xcache_2.0.1.sh安装xcache 2.0.1。(命令：./xcache_2.0.1.sh)
   （推荐安装）执行脚本xcache_3.0.3.sh安装xcache 3.0.3。(命令：./xcache_3.0.3.sh)
...
新增update.sh脚本，目的是为了自动检测和升级PHP、phpMyAdmin。这两种软件版本更新比较频繁，因此才会有此脚本，一劳永逸。
安装完lamp.sh一段时间后，你发现PHP或phpMyAdmin官网已更新，那即可运行此脚本更新到最新版。
修改：PHP5.5.x系列Release没多久，待测试其兼容性后再做升级，因此该脚本升级PHP的版本为5.4.x系列的最新版。
使用方法：
...
如果你在安装后使用遇到问题，请访问http://teddysun.com/lamp，提交评论，我会及时回复。
