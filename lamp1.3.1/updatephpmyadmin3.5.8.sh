...
cur_dir=`pwd`
cd $cur_dir
#download phpMyAdmin-3.5.8
if [ -s phpMyAdmin-3.5.8-all-languages.tar.gz ]; then
  echo "phpMyAdmin-3.5.8-all-languages.tar.gz [found]"
else
  echo "phpMyAdmin-3.5.8-all-languages.tar.gz not found!!!download now......"
  if ! wget http://teddysun.googlecode.com/files/phpMyAdmin-3.5.8-all-languages.tar.gz;then
    echo "Failed to download phpMyAdmin-3.5.8-all-languages.tar.gz,please download it to $cur_dir directory manually and rerun the install script."
 exit 1
 fi
fi

#install phpMyAdmin-3.5.8
echo "============================phpMyAdmin-3.5.8 install start================================"
rm -rf $cur_dir/untar/
mkdir -p $cur_dir/untar/
tar -xzf phpMyAdmin-3.5.8-all-languages.tar.gz -C $cur_dir/untar/
cp -f /data/www/default/phpmyadmin/config.inc.php $cur_dir/untar/config.inc.php.backup
rm -rf /data/www/default/phpmyadmin
mv $cur_dir/untar/phpMyAdmin-3.5.8-all-languages /data/www/default/phpmyadmin
